...
dirs.forEach(dir => {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    if (toolIdMatch) {
      ...
      const newDescription = toolDescriptions[toolId];
      
      if (newDescription) {
        // Replace existing description - multiline regex
        const descRegex = /<div class="tool-description">[\s\S]*?<\/div>/;
        const newDescHtml = `<div class="tool-description">\n          <p>${newDescription}</p>\n        </div>`;
        
        if (descRegex.test(content)) {
          content = content.replace(descRegex, newDescHtml);
          fs.writeFileSync(indexPath, content);
          console.log(`Updated: ${dir}/index.html`);
        } else {
          console.log(`No description found: ${dir}/index.html`);
        }
      }
    }
  }
...
console.log('\nDone! Enhanced all tool descriptions with more content.');
